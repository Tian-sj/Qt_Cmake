...
void values_survive_reopening_and_preserve_utf8() {
    ...
    {
        qtcpp::platform::FileSettingsStore settings{file};
        settings.set("用户.名称", value);
    }
    {
        qtcpp::platform::FileSettingsStore settings{file};
        qtcpp::test::require_equal(settings.get("用户.名称").value_or(""), value,
                                   "UTF-8 value must round-trip");
        settings.remove("用户.名称");
        qtcpp::test::require(!settings.get("用户.名称"), "removed setting must be absent");
    }
}
