...
using cppproject::precision_countdown::PrecisionCountdown;
using cppproject::precision_countdown::Signal;
...
void signal_connections_disconnect_safely() {
    ...
    Signal<int>::Connection connection;
    {
        ...
        signal.publish(2);
        cppproject::test::require_equal(calls, 2, "subscriber should be called");
        connection.disconnect();
        signal.publish(2);
        cppproject::test::require_equal(calls, 2, "disconnected subscriber should not be called");
    }
    connection.disconnect();
}
...
void zero_duration_finishes() {
    ...
    cppproject::test::require(completed, "zero-duration timer should finish promptly");
}
...
void pause_and_resume_preserve_remaining_time() {
    ...
    cppproject::test::require(countdown.paused(), "timer should be paused");
    cppproject::test::require((paused_remaining - still_remaining) < 5ms,
                              "remaining time should not advance while paused");

    countdown.resume();
    cppproject::test::require(countdown.running(), "timer should resume");
}
